...
**macOS 13 이상**에서 Markdown 문서를 읽는 네이티브 앱입니다. Swift와 AppKit으로 만들었으며 웹뷰나 외부 패키지를 사용하지 않습니다. Finder에서 파일을 열고, 접을 수 있는 사이드바에서 문서를 선택하며 읽을 수 있습니다. 라이트·다크 모드와 PDF 내보내기도 지원합니다. PDF를 만드는 동안에도 문서를 읽을 수 있고, 진행 상황을 확인하거나 작업을 취소할 수 있습니다. 사이드바에서 문서를 제거해도 원본 파일은 삭제되지 않습니다. 설치할 때 Finder의 다음으로 열기 목록에 앱을 등록합니다. 전체 화면에서는 상단 조작부가 숨겨지며, 화면 맨 위로 포인터를 옮기면 다시 나타납니다.
